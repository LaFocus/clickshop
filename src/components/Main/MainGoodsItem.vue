...
const itemToLocal = (e, event) => {
    const existingIndex = JSON.parse(localStorage.itemsGoods).findIndex(item => JSON.stringify(item) === JSON.stringify(e));
    console.log(existingIndex);
    if (existingIndex !== -1) {
        localItems.value.splice(existingIndex, 1);
    } else {
        localItems.value.push(e);
        localItems.value[localItems.value.indexOf(e)].active = true
    }
